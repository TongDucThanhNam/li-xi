...
function regretBadgeText(rarity: Rarity) {
	if (rarity === "legend") {
		return "Lỡ mất LEGEND";
	}
	return "Suýt trúng RARE";
}
...
export const EnvelopeCard = forwardRef<HTMLDivElement, EnvelopeCardProps>(
	function EnvelopeCard(
		{ card, index, isReady, isDetached = false, onClick },
		ref,
	) {
		const missedRarity = card.isMissed ? card.prize?.rarity ?? null : null;
		const isRegretMiss = missedRarity === "legend" || missedRarity === "rare";
		const entryMotionClass = card.isIn
			? "translate-y-0"
		...
		return (
			<div
				ref={ref}
				className={`group relative w-32 sm:w-36 md:w-40 lg:w-44 aspect-[0.72] preserve-3d cursor-pointer transition-[opacity,transform] duration-600 ease-elastic ${entryMotionClass} ${
					isDetached ? "!opacity-0 pointer-events-none" : ""
				} ${
					card.isMissed
						? missedCardClass(missedRarity)
						: ""
				}`}
				onClick={onClick}
				role="button"
				tabIndex={
					!isDetached && isReady && !card.isOpened && !card.isMissed ? 0 : -1
				}
			...
			>
				...
				>
					{/* Lining */}
					<div className="layerLining absolute inset-0 rounded-md -translate-z-px bg-liner-dark shadow-[0_10px_30px_rgba(0,0,0,0.5)] bg-[repeating-linear-gradient(45deg,rgba(0,0,0,0.2)_0,rgba(0,0,0,0.2)_1px,transparent_1px,transparent_4px)]" />
					...
					>
						...
						{card.isOpened && card.prize ? (
							...
							>
								...
								(card.prize.rarity === "legend" ||
									card.prize.rarity === "rare")
									? regretBadgeText(card.prize.rarity)
