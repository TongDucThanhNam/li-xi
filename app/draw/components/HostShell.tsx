...
export default function HostShell({ children }: { children: ReactNode }) {
	const pageBackground: CSSProperties = {
		backgroundImage: [
			"radial-gradient(circle at 8% 0%, rgba(179, 20, 20, 0.42), transparent 40%)",
			"radial-gradient(circle at 90% 4%, rgba(212, 175, 55, 0.2), transparent 38%)",
			"radial-gradient(circle at 86% 86%, rgba(94, 10, 10, 0.46), transparent 52%)",
			"radial-gradient(circle at 35% 22%, rgba(255, 248, 220, 0.12), transparent 44%)",
			"linear-gradient(150deg, var(--color-black-ink) 0%, var(--color-liner-dark) 48%, var(--color-red-deep) 100%)",
		].join(", "),
	};
	...
	return (
		...
		>
			...
			/>
			<div
				className="pointer-events-none absolute -left-[10%] -top-[20%] h-[800px] w-[800px] rounded-full opacity-40 blur-[120px] mix-blend-screen animate-float-slow"
				style={lightOneStyle}
			/>
			<div
				className="pointer-events-none absolute -bottom-[20%] -right-[10%] h-[600px] w-[600px] rounded-full opacity-[0.25] blur-[100px] mix-blend-screen animate-float-medium"
				style={lightTwoStyle}
			/>
			<div
				className="pointer-events-none absolute left-[40%] top-[40%] h-[500px] w-[500px] rounded-full opacity-[0.15] blur-[100px] mix-blend-overlay animate-pulse-slow"
