...
import {Sidebar, Command, Sheet} from "@heroui-pro/react";
...
- Switch (and matching Checkbox/Radio in @heroui/react v3.2+) must use the dot-notation anatomy with `Switch.Control > Switch.Thumb` nested **inside** `Switch.Content`; the label is plain text inside `Switch.Content` (never nest a `<Label>` component). Place Description/FieldError as siblings of `Switch.Content`. Never use the old `<Switch>Label</Switch>` or the v3.1 `Control + Content` sibling layout.
